...
function Keyfob({ setProduct }) {
	...
	const getBracelets = async () => {
		const resp = await axios.get(`${BASE_URL}/keyfob`);
		if (resp.status === 200) {
	...
	const singleSelectedItems = ["keyfobmaterial", "keyfobshape", "rfid"];

	const displayItemStyle = (item, itemName, itemPrice) => {
		if (item.name === "keyfob material") setKeyfobMaterial({ display: true, itemName: itemName, itemPrice: itemPrice });
		if (item.name === "keyfob shape") setKeyfobShape({ display: true, itemName: itemName, itemPrice: itemPrice });
	...
	const StickerStyle = () => {
		return (
			<div className="keyfob-style">
				<div className="keyfob-object">
					<div className="keyfob-circle">
						{backgroundImage.display && <BackgroundImageStyle />}
						<div className="keyfob-keyround"></div>
						<div className="keyfob-key">
							<div className="keyfob-key-blending"></div>
						</div>
						<div className="keyfob-square-cut"></div>
					...
					<div className="keyfob-square"></div>
				</div>
				{keyfobMaterial.display && <KeyfobMaterialStyle />}
				{keyfobShape.display && <KeyfobShapeStyle />}
			</div>
		);
	};

	const [keyfobMaterial, setKeyfobMaterial] = useState({});
	const KeyfobMaterialStyle = () => {
		return (
			<div className="keyfob-material">
				<TooltipLeft toolTipStyle={{ left: 316, top: 44 }} tooltipStickStyle={{ width: 51 }} item={keyfobMaterial} quantity={quantity} />
			</div>
		);
	};

	const [keyfobShape, setKeyfobShape] = useState({});
	const KeyfobShapeStyle = () => {
		return (
			<div className="keyfob-shape">
				<TooltipLeft toolTipStyle={{ left: 363, top: 85 }} tooltipStickStyle={{ width: 41 }} item={keyfobShape} quantity={quantity} />
			</div>
		);
	};
	...
	const LogoStyle = () => {
		return (
			<div className="keyfob-wlogo">
				<div className="keyfob-wlogo-style"></div>
				<TooltipBottom toolTipStyle={{ left: 70, top: -52 }} tooltipStickStyle={{ height: 75 }} item={logo} quantity={quantity} />
			</div>
		);
	};
	...
	const QrcodeStyle = () => {
		return (
			<div className="keyfob-qr-code">
				<div className="keyfob-qrcode-style"></div>
				<TooltipTop toolTipStyle={{ top: 158, left: 27 }} tooltipStickStyle={{ height: 120 }} item={qrCode} quantity={quantity} />
			</div>
		);
	};
	...
	const BarCodeStyle = () => {
		const _barCodeShortNames = barCode.itemList.selected
			...
			.join(" | ");
		return (
			<div id="keyfob-bar-code" className="keyfob-bar-code">
				<div className="keyfob-bar-code-style"></div>
				<TooltipTop toolTipStyle={{ left: 92, top: 175 }} tooltipStickStyle={{ height: 61 }} item={{ ...barCode, itemName: _barCodeShortNames }} quantity={quantity * barCode.itemList.selected.length} />
			</div>
		);
	};
	...
	const TextStyle = () => {
		return (
			<div className="keyfob-text">
				<p className="keyfob-text-style">text</p>
				<TooltipLeft toolTipStyle={{ left: 154, top: 154 }} tooltipStickStyle={{ width: 41 }} item={text} quantity={quantity} />
			</div>
		);
	};
	...
	const ChipNumberStyle = () => {
		return (
			<div className="keyfob-chip-number">
				<p className="keyfob-chip-number-style">123-4567</p>
				<TooltipTop toolTipStyle={{ top: 160, left: 129 }} tooltipStickStyle={{ height: 35 }} item={chipNumber} quantity={quantity} />
			</div>
		);
	};
	...
	const RfidStyle = () => {
		return (
			<div className="keyfob-rfid">
				<div className="keyfob-rfid-style">
					<div className="keyfob-rfid-chip"></div>
					<div className="keyfob-rfid-chip-stick-1"></div>
					<div className="keyfob-rfid-chip-stick-2"></div>
				</div>
				<TooltipLeft toolTipStyle={{ left: 148, top: 74 }} tooltipStickStyle={{ width: 61 }} item={rfid} quantity={quantity} />
			</div>
		);
	};
	...
	const BackgroundImageStyle = () => {
		return (
			<div id="keyfob-back-image" className="keyfob-back-image">
				<div className="keyfob-background-border">
					<div className="keyfob-back-image-style"></div>
				</div>
				<TooltipLeft toolTipStyle={{ left: 188, top: -87 }} tooltipStickStyle={{ width: 40 }} item={backgroundImage} quantity={quantity} />
			</div>
		);
	};
